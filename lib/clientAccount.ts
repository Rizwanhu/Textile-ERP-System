...
/** Prefer slug for URLs (e.g. client_scotland), fall back to UUID id. */
export function clientRouteId(client: Client): string {
  return client.slug ?? client.id;
}

export function findClientByRef(clients: Client[], ref: string): Client | undefined {
  return clients.find((c) => c.id === ref || c.slug === ref);
}
...
export function getClientAccountBundle(data: ClientAccountData, clientRef: string) {
  const client = findClientByRef(data.clients, clientRef);
  if (!client) return undefined;
  const lineItems = data.lineItems.filter((l) => l.clientId === client.id);
  const invoices = data.invoices.filter((i) => i.clientId === client.id);
  const payments = data.payments.filter((p) => p.clientId === client.id);
  const creditNotes = data.creditNotes.filter((c) => c.clientId === client.id);
