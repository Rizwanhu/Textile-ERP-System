...
import { upsertMaterialRequirement } from "@/actions/materials";
...
export function NewMaterialDialog({ open, onOpenChange, defaultOrderId, onCreated }: Props) {
  const { orders: materialOrders } = useMaterials();
  ...
  useEffect(() => {
    ...
    else if (materialOrders[0] && !form.orderId) {
      setForm((p) => ({ ...p, orderId: materialOrders[0].orderId }));
    }
  }, [defaultOrderId, materialOrders, form.orderId]);
  ...
  const validate = (): string | null => {
    ...
    if (isLocalBuyer && !form.supplier.trim()) return "Local Supplier source needs a supplier.";
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const result = await upsertMaterialRequirement({
      ...
      source: form.source,
      supplier: isLocalBuyer ? form.supplier : undefined,
    });
    if ("error" in result) {
      toast({ title: "Could not save material", description: result.error, variant: "destructive" });
      return;
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[92vh] w-[calc(100vw-1.5rem)] max-w-[640px] flex-col overflow-hidden bg-card border-border p-0 sm:w-full">
        <DialogHeader className="border-b border-border px-5 py-4 sm:px-6">
          ...
          <DialogDescription>
            Every material must be sourced — from the Local Supplier (processed fabric / thread) or pulled from existing stock.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2 mt-2 rounded-lg border border-border bg-surface-1/60 p-3">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  {isLocalBuyer ? (
                    <Field label="Supplier (Local Supplier)" required>
                      <Select value={form.supplier} onValueChange={(v) => setForm({ ...form, supplier: v })}>
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="__new__">+ Add as new supplier in Local Supplier</SelectItem>
                        </SelectContent>
                      </Select>
                    </Field>
                  ) : (
                ...
                <p className="mt-2 flex items-start gap-1.5 text-[11px] text-muted-foreground">
                  ...
                  {isLocalBuyer
                    ? "This entry will appear under Expenses → Local Supplier for the chosen supplier."
                    : "Existing stock won't create a Local Supplier entry — it just decrements your inventory."}
                </p>
              </div>
