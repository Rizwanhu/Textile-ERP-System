...
export function CreateInvoiceDialog({ open, onOpenChange, client, lineItems, selectedIds, onSuccess }: Props) {
  ...
  const submit = async () => {
    ...
    const result = await createInvoice(client.id, selected.map((l) => l.id), notes || undefined);
    if ("error" in result) {
      toast({ title: "Could not create invoice", description: result.error, variant: "destructive" });
      return;
    }
    const invoice = result;
    const bundle = getBundle(client.slug ?? client.id);
    if (bundle) {
