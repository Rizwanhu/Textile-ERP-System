...
export function NewOrderDialog({ open, onOpenChange, onCreated }: Props) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if ("error" in result) {
        toast({ title: "Could not create order", description: result.error, variant: "destructive" });
        return;
      }
        ...
        <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col">
         <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4 sm:px-6">
          ...
          <DialogFooter className="gap-2 border-t border-border bg-surface-1/40 px-5 py-3 sm:px-6">
            <Button type="button" variant="outline" className="border-border" onClick={() => onOpenChange(false)} disabled={loading}>Cancel</Button>
            <Button type="submit" className="bg-primary text-primary-foreground hover:bg-primary-hover" disabled={loading}>
