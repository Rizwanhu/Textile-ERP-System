...
export function ClientFormDialog({ open, onOpenChange, client, onCreated }: Props) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (isEdit && client) {
      ...
      if (result.error) {
        toast({ title: "Could not update client", description: result.error, variant: "destructive" });
        return;
      }
      ...
      if ("error" in result) {
        toast({ title: "Could not add client", description: result.error, variant: "destructive" });
        return;
      }
