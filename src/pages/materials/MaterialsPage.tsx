...
import {
  MATERIAL_ORDERS, MATERIAL_SOURCES, type MaterialOrderSummary, type MaterialRequirement,
...
const MaterialsPage = () => {
  const [activeOrderId, setActiveOrderId] = useState(MATERIAL_ORDERS[0].orderId);
  const [newOpen, setNewOpen] = useState(false);
  const order = useMemo(
    () => MATERIAL_ORDERS.find((o) => o.orderId === activeOrderId) ?? MATERIAL_ORDERS[0],
    [activeOrderId],
  );
  ...
  return (
    <div className="mx-auto max-w-[1440px] animate-fade-in p-4 lg:p-6">
      ...
      <NewMaterialDialog open={newOpen} onOpenChange={setNewOpen} defaultOrderId={order.orderId} />
            ...
            <CardHeader className="flex flex-row items-start justify-between gap-4 pb-4">
              <div>
                <div className="mb-1 flex items-center gap-2">
                  <Link to={`/orders/${order.orderId}`} className="mono text-sm text-primary hover:underline">
                    {order.orderId}
                  </Link>
...
function DefineStep({ order }: { order: MaterialOrderSummary }) {
  ...
  const sourceLabel = (key: string) =>
    MATERIAL_SOURCES.find((s) => s.key === key)?.label.replace("Local Buyer · ", "") ?? key;
  return (
    <Card className="border-border bg-card shadow-card">
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-border hover:bg-transparent">
              ...
              <TableHead className="w-[200px]">Fulfillment</TableHead>
